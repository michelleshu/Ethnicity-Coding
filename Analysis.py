from numpy import *                                       # representing arrays of numbers and characters efficiently
from sklearn.ensemble import GradientBoostingClassifier   # existing implementation of gradient-boosted classifier
...
# evaluation data is data for which there is no known label - this is the real application of the classifier.
# "Evaluation.dat" is generated by the R helper function, so it is known to exist
def loadEvaluate():
    with open('Evaluation.dat','r') as f:  # open the file containing the data for which we have no labels
        # parse each line, removing quotation marks and converting everything to lower case
        evaluation = [line.strip().replace('"','').lower() for line in f]
        f.close()
    return evaluation  # return a list of the surnames


# similar to the function loadEvaluate, but extracts data for which we have known labels and confidences.
# the files are constructed by the R helper function, so it is known to exist
def loadTrainTest():
    # for all files containing names, confidences, and labels, strip quotation marks,
    # and convert to lower case while parsing
    ...
    with open('Ethnicity.dat','r') as f:
        ...
        f.close()
    # return a list containing, in order, a surname, its confidence, and its known label
    return names, confidence, ethnicity
...
# train and test a gradient-boosted decision tree classifier. takes as input a training set and a testing set and
# returns the accuracy, the prediction, and the classifier object itself
def createGB(repTr,repTe,labelTr,labelTe):
    # train the classifier with 100 trees
    c = GradientBoostingClassifier(n_estimators=20, learning_rate=1.0, max_depth=5, random_state=0).fit(repTr,labelTr)
    p = atleast_2d(c.predict(repTe)).T          # generate predictions of the classifier for the test data set and
                                                # convert into numpy 2-D array
...
# train and test a random forest classifier. this function is virtually identical to the function createGB,
# except that the classifier it constructs is different
def createRF(repTr,repTe,labelTr,labelTe):
    c = RandomForestClassifier(n_estimators=100, n_jobs=-1, max_depth=None, min_samples_split=7,
                               random_state=0).fit(repTr, labelTr)
...
def partitionTrainTest(rep,label):
    index = random.permutation(rep.shape[0])  # randomly reindex the data (this is good practice so as to avoid
                                              # potential correlations that make classification unfair)
    rep, label = rep[index,:], label[index]   # perform reindexing
    # l = round(.75 * rep.shape[0])           # proportion of data is used for training the classifier
                                              # - presently set to 75% of the data
    ...
    labelTr, labelTe = label[0:l], label[(l+1):,:]  # obtain the corresponding labels for training and testing
    # return a data set that has been partitioned into training and testing components
    return repTr, repTe, labelTr, labelTe
...
def writeResults(names,gbl,rfl,f):
    d = [names,gbl,rfl]  # set up list to write to file - first column is surname, the gradient boosted and
                         # random forest classificcations, respectively
...
def main():
    ...
    # take as input from the user both a data file of individuals with known labels and unknown labels
    command = 'Rscript Process.r --inputfiles ' + sys.argv[1] + ' ' + sys.argv[2]
    os.system(command)  # pass this as a system call, which invokes an R script intended to easily generate files
                        # without missing data, etc.
    ...
    # extract numeric ethnicity labels from corresponding strings for each individual
    label = createLabels(ethnicity, d)

    # partition the labeled data into training and testing components
    repTr, repTe, labelTr, labelTe = partitionTrainTest(rep, label)
    print 'successfully partitioned the data into training and testing components...'
    # construct vector representation for the unlabeled data - can neither be trained upon nor tested upon -
    # potential for self-taught learning?
    repEval = evaluationVectors(evaluation, ngrams)
    ...
    if gbflag:
        ...
        print gba
        if True:
            # convert the predictions of the classifer, which are integers, back into the strings that specify ethnicity
            gbl = retrieveClass(gbp, dinv)
    if rfflag:
        ...
        print rfa
        if True:
            # convert the predictions of the classifier, which are integers, back into the strings that specify ethnicity
            rfl = retrieveClass(rfp,dinv)
    if not gbflag: 
        gbl = array([])
    if not rfflag:
        rfl = array([])
    # write the results of the classification process to a csv file "TestingResults.csv"
    writeResults(names, gbl, rfl, 'TestingResults.csv')
    if False:
        # for the unlabeled data, obtain a predicted class for the gradient boosted classifier and the random forest
        gbEval = atleast_2d(gbc.predict(repEval)).T
